...
export class LevelUpStep extends TutorialStep {
    buildContent(): void {
        ...
        const title = this.scene.add.text(centerX, currentY, 'You Leveled Up! 🎉', 
        ...
        const welcomeText = this.scene.add.text(centerX, currentY, 
            'Look, you\'ve leveled up! You\'re now a bit stronger than you were.\n\nWhen you die, you\'ll get to choose rewards to power yourself up even more. Try dying to see what\'s next!', 
        ...
        xpArc.beginPath();
        xpArc.arc(xpX, xpY, config.RADIUS, -Math.PI / 2, -Math.PI / 2 + (2 * Math.PI)); // Full circle
        xpArc.strokePath();
        ...
        this.contentContainer.add(level2Text);
