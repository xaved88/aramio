...
export class MovementStep extends TutorialStep {
    buildContent(): void {
        ...
        const title = this.scene.add.text(centerX, currentY, 'Movement', 
        ...
        const text = this.scene.add.text(centerX, currentY, 
            '• Use WASD keys to move your hero\n' +
            '• Right-click to toggle mouse-follow mode\n' +
            '• Hold Space or Left-click to aim abilities', 
        ...
        const nextText = this.scene.add.text(centerX, currentY, 'Continue', 
            TextStyleHelper.getStyle('BUTTON_TEXT'));
